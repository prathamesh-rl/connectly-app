# ---------------------------------------------------------------
#  Connectly Messaging Dashboard  •  robust file-discovery build
# ---------------------------------------------------------------
...
# ───────── Helper: build brace-list for DuckDB ─────────
def brace_list(patterns, label):
    """
    Return '{file1,file2,...}' ready for DuckDB parquet_scan.
    Stop with Streamlit error if none of the patterns match.
    """
    for pat in patterns:
...
# ─────────── File patterns (edit if layout changes) ──────────
CAMPAIGN_PATTERNS = [
    "parquet_trim/dispatch_date=*/data_0.parquet",
    "parquet_output/dispatch_date=*/data_0.parquet",
]
ACTIVITY_PATTERNS = [
    "activity_chunks/activity_data_*.parquet",  # your 120 MB set
    "activity_trim/act_*.parquet",              # fallback
]
...
def monthly_sent_delivered():
    ...
    FROM parquet_scan('{PARQUET_FILES}')
...
def load_msgs(sd, ed):
    ...
    FROM parquet_scan('{PARQUET_FILES}') m
...
def load_act(sd, ed):
    ...
    FROM parquet_scan('{ACTIVITY_FILES}')
...
merged = nudges.merge(active, left_on="customer_external_id", right_on="user_phone", how="left").fillna(0)
...
for b in bars:
    ax_h.text(b.get_x()+b.get_width()/2, b.get_height()+.5, f"{b.get_height():.0f}%",
              ha='center', color=TXT)
