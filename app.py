# ---------------------------------------------------------------
# Connectly Messaging Dashboard  •  June-2025  (stable build)
# ---------------------------------------------------------------
...
# ---------- File patterns (with fallbacks) ----------
CAMPAIGN_PATTERNS = [
    # matches every data_0.parquet inside each dispatch_date=… folder
    "parquet_trim/dispatch_date=*/data_0.parquet",

    # fallback to the original un-trimmed set
...
def first_match(pats):                     # return first pattern that has files
    for p in pats:
        if glob.glob(p):
            return p
    return None

PARQUET_PATH  = first_match(CAMPAIGN_PATTERNS)
ACTIVITY_PATH = first_match(ACTIVITY_PATTERNS)
...
def monthly_sent():
    ...
    SELECT DATE_TRUNC('month', CAST(dispatched_at AS TIMESTAMP))::DATE AS m,
           COUNT(DISTINCT customer_external_id||'_'||CAST(CAST(dispatched_at AS TIMESTAMP)::DATE AS VARCHAR)) AS sent,
           COUNT(DISTINCT CASE WHEN delivered IS NOT NULL
                 THEN customer_external_id||'_'||CAST(CAST(dispatched_at AS TIMESTAMP)::DATE AS VARCHAR) END) AS delivered
    FROM parquet_scan('{PARQUET_PATH}')
    GROUP BY 1 ORDER BY 1;
    """
    df = duckdb.sql(q).df()
    df["label"] = pd.to_datetime(df["m"]).dt.strftime("%b %y")   # Jan 25 style
...
def load_msgs(sd, ed):
    ...
    SELECT DISTINCT
           ...
           CAST(CAST(dispatched_at AS TIMESTAMP)::DATE AS DATE) AS msg_date,
           customer_external_id||'_'||CAST(CAST(dispatched_at AS TIMESTAMP)::DATE AS VARCHAR) AS message_id,
           delivered, button_responses, link_clicks, sendout_name,
           COALESCE(mp.product,'Unknown') AS product
    FROM parquet_scan('{PARQUET_PATH}') m
...
def load_act(sd, ed):
    if ACTIVITY_PATH is None:
        return pd.DataFrame(columns=["user_phone","product","activity_date"])
    q = f"""
    SELECT user_phone, product, CAST(activity_date AS DATE) AS activity_date
    FROM parquet_scan('{ACTIVITY_PATH}')
    WHERE activity_date::DATE BETWEEN DATE '{sd}' AND DATE '{ed}';
    """
    return duckdb.sql(q).df()

# ---------- Monthly charts ----------
msum, mcost = monthly_sent(), monthly_cost()
st.subheader("📈 Monthly Messaging & Cost Overview")

fig,(ax1,ax2) = plt.subplots(1,2, figsize=(12,4), facecolor=BG)

x, w = range(len(msum)), .35
ax1.bar([i-w/2 for i in x], msum["sent"],      w, color="#00b4d8")
ax1.bar([i+w/2 for i in x], msum["delivered"], w, color="#ffb703")
...
start_date = col_s.date_input("Start date", max_d-datetime.timedelta(days=30),
                              min_value=min_d, max_value=max_d)
end_date   = col_e.date_input("End date",   max_d, min_value=min_d, max_value=max_d)
...
tot_counts = funnel.iloc[:,1:5].sum()
tot_row = pd.DataFrame([{
    "product": "Total",
    **tot_counts
}])
funnel = pd.concat([funnel, tot_row], ignore_index=True)
...
merged = pd.merge(nudges, active, how="left",
                  left_on="customer_external_id", right_on="user_phone").fillna(0)
...
merged["segment"] = pd.cut(
    merged["days"],
    bins=[-1,0,10,merged["days"].max()],
    labels=["Inactive","Active","Highly Active"]
)

seg_tbl = pd.merge(
    msgs_sel[["customer_external_id","sendout_name"]],
    merged[["customer_external_id","segment"]],
    how="left", on="customer_external_id"
)

ct = pd.crosstab(seg_tbl["sendout_name"], seg_tbl["segment"], normalize="index")
ct = ct.reindex(columns=["Inactive","Active","Highly Active"], fill_value=0)
ct *= 100
seg_pct = ct.reset_index()

camp_final = camp.merge(seg_pct, on="sendout_name", how="left").round(1)

# add '%' symbol
for col in ["Inactive","Active","Highly Active"]:
    camp_final[col] = camp_final[col].fillna(0).round(1).astype(str) + "%"
